...
const GameScreen1 = () => {
  ...
  const nodes = [
    ...
    { id: 'B', x: width * 0.75, y: height * 0.3 },
    { id: 'C', x: width * 0.25, y: height * 0.3 },
    { id: 'D', x: width * 0.5, y: height * 0.5 },
    { id: 'E', x: width * 0.3, y: height * 0.75 },
    { id: 'F', x: width * 0.7, y: height * 0.75 },
  ];
  ...
  useEffect(() => {
    const initialConnections = [
      ...
      { start: 'B', end: 'D', weight: Math.floor(Math.random() * 10 + 1) },
      { start: 'C', end: 'D', weight: Math.floor(Math.random() * 10 + 1) },
      ...
      { start: 'D', end: 'A', weight: Math.floor(Math.random() * 10 + 3) },
    ];
  ...
  const handleSubmit = () => {
    ...
    if (shortestPath.trim() === correctPath) {
      setTimeout(() => {
        setLoading(false); 
        navigation.navigate('ResultScreen', { result: 'correct', path: correctPath });
      }, 1500); // 1.5 saniye
    } else {
      setTimeout(() => {
        setLoading(false); 
        navigation.navigate('ResultScreen', { result: 'incorrect', path: correctPath });
      }, 1500); 
    }
  };
